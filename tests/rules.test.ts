...
describe('shot rules', () => {
  it('scores a clean shot', () => expect(evaluateShot([{x:0,y:50},{x:50,y:5}],[],keeper,goal)).toBe('goal'));
  it('blocks before goal', () => expect(evaluateShot([{x:0,y:50},{x:50,y:5}],[{center:{x:20,y:32},radius:5}],keeper,goal)).toBe('blocked'));
  it('saves at keeper', () => expect(evaluateShot([{x:0,y:50},{x:0,y:20},{x:50,y:5}],[],keeper,goal)).toBe('saved'));
